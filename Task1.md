...
Python 语言支持逻辑运算符，假设两变量：a = 5, b = 2：

| 运算符 | 逻辑表达式 | 描述                                                         | 实例             |
| ------ | ---------- | ------------------------------------------------------------ | ---------------- |
| and    | a and b    | 布尔“与”，如果 a 为 False，a and b 返回 False，否则返回 b 的计算值 | (a and b) 返回 2 |
| or     | a or b     | 布尔“或”，如果 a 是非 0，返回 a 的值，否则返回 b 的计算值    | (a or b) 返回 5  |
| not    | not a      | 布尔“非”，如果 a 为True，返回 False。如果 a 为 False，返回 True | not a 返回 False |

 ## 4) 成员运算符

Python支持成员运算符，成员包括字符串，列表或元组

假设: a = 1, b = [1, 2, 3]

| 运算符 | 描述                                                 | 实例                  |
| ------ | ---------------------------------------------------- | --------------------- |
| in     | 如果 a 在列表 b 中找到值，返回 True，否则返回 False  | a in b 返回 True      |
| not in | 如果 a 在列表 b 中找到值，返回 False，否则返回 False | a not in b 返回 False |

 ## 5) 身份运算符

身份运算符用于比较两个对象的存储单元，即内存 id 地址
...
| 运算符   | 描述                               | 实例                                                         |
| -------- | ---------------------------------- | ------------------------------------------------------------ |
| is       | 判断两个标识符是不是引用自一个对象 | a is b，如果 id(a) == id(b) 为 True，返回 True。否则返回 False |
| is not b | 判断两个标识符是不是引用自不同对象 | a is not b，如果 id(a) == id(b) 为 False，返回 True。否则返回 False |
